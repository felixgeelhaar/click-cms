 ...
 * optional. What lives here is the *management* API for pages, which is not.
 */
final class PagesController
{
     ...
     *
     * A media id this application generates ends in a dash and eight hex digits.
     * It appears both as a whole field value (an image field) and embedded in
     * text — a rich-text or Markdown body with an inline image. Both are matched,
     * so a headless front end reading a body can resolve its images to variants
     * without a second request. A pattern match that is not a real media id
     * resolves to nothing and is dropped, so a false positive costs a lookup, not
     * a wrong image.
     *
    ...
    private function resolveMediaReferences(mixed $sections): array
    {
        ...
        $resolved = [];
        foreach (self::mediaIdsIn($sections) as $id) {
    ...
    /**
     * Every distinct media id referenced anywhere in a nested content structure —
     * as a whole field value or embedded in a text/Markdown body — in first-seen
     * order. Pure and static, so the matching is testable without a booted kernel
     * or a media directory.
     *
     * @return list<string>
     */
    public static function mediaIdsIn(mixed $value): array
    {
        $ids = [];
        $scan = static function (mixed $node) use (&$ids, &$scan): void {
            if (is_array($node)) {
            ...
            // Bounded so the eight hex digits are exactly eight and the id starts
            // at a token boundary — an id inside a URL or a Markdown link still
            // matches, a longer hex run does not.
            if (preg_match_all('/(?<![a-z0-9-])[a-z0-9][a-z0-9-]*-[a-f0-9]{8}(?![a-f0-9])/', $node, $m) >= 1) {
                foreach ($m[0] as $id) {
                    $ids[$id] = true;
                }
            }
